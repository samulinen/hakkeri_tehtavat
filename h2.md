...
010-staff-only
- Koitin Debian12 ratkaista. Käytin komentoja 'SELECT * FROM pins WHERE pin='123'', 'SELECT * FROM foo WHERE true', 'SELECT * FROM foo WHERE true OR 1=1--', OR 1=1--. Mikään ei toiminut enkä saanut vinkeistä apua. Johdantotehtävät sain ratkaistua, tein ne uudestaan tiistain luennon jälkeen. Joten koitin soveltaa niiden ratkaisuja, mutta ei tosiaan tuottanut tulosta.
